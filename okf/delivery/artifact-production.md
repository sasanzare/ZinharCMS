...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Debug backend binary | Backend crate | `cargo build` | Rust toolchain | `backend/target/debug` | Crate `3.0.0`; file path not versioned | Local Cargo behavior; ignored | Developer | Cargo compilation only | `VERIFIED` |
| Release backend binary | Backend crate | `cargo build --release` | Rust 1.96 builder in production Dockerfile | Builder `target/release/cms-backend`; copied into image | Crate `3.0.0`; image tag undefined | Not defined | Backend runtime image | Docker build/Cargo; no checksum/signing | `VERIFIED` build, retention unknown |
| Frontend bundle | Frontend source | `npm run build` | Node/npm; Node 24 in production builder | `frontend/dist`; copied to Nginx image | Frontend `3.0.0`; filenames may be hashed by Vite | CI output is ephemeral; local ignored | Nginx/static host | Build success; no published checksum | `VERIFIED` |
