...
| Browser end-to-end | No committed command; Phase 4 used the Codex in-app browser against disposable local services | Browser runtime | Production build, PostgreSQL, Redis, backend | Disposable local credentials/data | Dedicated database and Redis DB; exact cleanup | No | DOM/header/console evidence | No committed Playwright/Cypress/browser CI matrix |
...
| Security | Backend rich-content tests, frontend security tests, `npm run security:sinks`, and Phase 4 browser matrix | Backend/frontend/root | Rust, Node, optional browser/services | Pure tests plus disposable browser environment | Shared malicious corpus and disposable database/Redis | Unit tests indirect; browser not in CI | Cargo/Vitest/AST/browser evidence | No complete IDOR, cross-tenant, replay, dependency-advisory, or fuzz suite |
