...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DEV-R01 | Toolchain support could drift | CI and Docker now align on Node 24 and Rust 1.96; host OS/browser support remains undefined | Local/CI/container drift | Low | MEDIUM | Lockfiles, `npm ci`, pinned CI versions, Cargo `rust-version`, and matching Docker major/minor versions | Keep versions synchronized and define the remaining OS/browser support matrix | Yes | `PARTIALLY_MITIGATED` |
...
| DEV-R09 | Frontend dependency install could become mutable again | CI, local guidance, and images now use `npm ci` with a tracked lockfile | Resolver drift or unnoticed lock changes | Low | LOW | Lockfile-enforced installation in all tracked build paths | Keep `npm ci` in CI/images and review lockfile changes | No | `MITIGATED` |
