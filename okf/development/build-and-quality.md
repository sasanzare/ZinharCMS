...
| Rust format | `cargo fmt --check` | Backend Rust | rustfmt from Rust 1.96 | Yes | Job fails | Run before tests | `VERIFIED` |
| Rust static analysis | `cargo clippy --all-targets --all-features -- -D warnings` | All backend targets/features | Clippy on Rust 1.96 | Yes | Any warning fails job | Run after format | `VERIFIED`; green for the V3 release candidate |
...
| Dependency integrity | Cargo lock and frontend lock are tracked | Backend/frontend dependency graph | Lock files | Backend uses locked Cargo commands; frontend CI/images/local guidance use `npm ci` | Resolver/install failure | Review lock diffs | `VERIFIED` tracked enforcement |
| Frontend dependency audit | `npm audit --audit-level=high` | Frontend production and development dependencies | npm advisory registry | Yes | High or critical finding fails CI | Run after `npm ci` | `VERIFIED`; zero findings for the V3 release candidate |
| Other security scanning | No SAST, container image scan, or secret-scan job found | Repository | None | No | Not applicable | Manual/owner decision | `PLANNED_NOT_IMPLEMENTED` |
...
- Frontend CI and both Dockerfiles enforce the tracked lockfile with `npm ci`.
