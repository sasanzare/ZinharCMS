...
| CMD-FE-01 | Dependencies | `npm ci` | `frontend/` | Install exactly the locked dependencies | Frontend CI/Dockerfiles/README | package files, registry | Recreates `node_modules`; fails on manifest/lock mismatch | Conditional | Does not update lockfile | Node/npm, registry | `ENVIRONMENT_DEPENDENT` |
...
| CMD-FE-07 | Preview | `npm run preview` | `frontend/` | Serve built bundle with production CSP/Trusted Types headers | Frontend manifest | Existing `dist` | Preview server | Yes | None | Prior build | `VERIFIED` locally in Phase 4 |
| CMD-FE-08 | Security policy | `npm run security:sinks` | `frontend/` | Enforce the single approved HTML sink with TypeScript AST analysis | Frontend manifest | Runtime TypeScript/TSX source | Nonzero on an unapproved sink | Yes | No source mutation | Installed dependencies | `VERIFIED` |
