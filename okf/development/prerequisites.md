...
| Rust toolchain and Cargo | Backend build, format, lint, test, run | Required for non-container backend work | CI and both backend images use Rust `1.96`; the crate declares `rust-version = "1.96"` and edition `2024` | Workflow, Dockerfiles, Cargo manifest | `dtolnay/rust-toolchain@1.96.0` in CI | Backend | `rustc --version`; `cargo --version`; backend CI gates | `VERIFIED` declaration and local validation |
