...
| `authentication.refresh` | **POST** `/api/auth/refresh` | `backend/src/routes/auth.rs#refresh` | Public/cookie + Origin boundary | headers | `Response`; 200 | Included | `api.auth.refresh` | [Contract](endpoints/authentication-and-session.md) |
| `authentication.logout` | **POST** `/api/auth/logout` | `backend/src/routes/auth.rs#logout` | Public/cookie + Origin boundary | headers | `Result<(HeaderMap, Json<LogoutResponse>), AppError>`; 200 | Included | `api.auth.logout` | [Contract](endpoints/authentication-and-session.md) |
...
| `pages-components-preview.issue-preview-ticket` | **POST** `/api/pages/{id}/preview-ticket` | `backend/src/routes/pages.rs#issue_preview_ticket` | Bearer + tenant + preview reader | path `Uuid` | `Result<Json<PreviewTicketResponse>, AppError>`; 200 | Included | `api.pages.previewTicket` | [Contract](endpoints/pages-workflow-versions-and-preview.md) |
...
| `pages-components-preview.preview-page` | **GET** `/api/preview/{page_id}` | `backend/src/routes/pages.rs#preview_page` | Exact Origin + one-time ticket subprotocol | path `Uuid`; no query; WebSocket upgrade | `Response`; 101 | Included | `previewSocket.ts` | [Contract](endpoints/pages-workflow-versions-and-preview.md) |
